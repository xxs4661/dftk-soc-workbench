...
| [Supplementary core and K6 record](../results/soc-extra/README.md) | All five performance samples per operation, fresh B0 endpoint arithmetic and the retained K6 resource-stop evidence; [command and exit scopes](../scripts/README.md#public-result-review). Default consistency does not imply K6 success; strict K6 mode requires qualified endpoints. | The underlying private arrays, profiler stacks and resource observations remain runner-reported. No converged K6 endpoint or new K6 D−QE comparison is available. |
