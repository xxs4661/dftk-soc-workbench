...
- Phase 9A continuation: the [fixed resume contract](../benchmarks/soc-core-memory-v1/resume/README.md)
  adds explicit immutable authorization and real nonphysical check-entry while
  preserving the default same-commit gate and global lock. Both newly authorized
...
- Phase 9A: `benchmarks/soc-core-memory-v1/run.py` runs one authenticated REF/CORE suite with the prescribed five samples. `run_si_dftk.py OPT-B0-SCF <new-directory> --backend soc-core --core-gate <gate.json> --execution-commit <exact-SHA>` requires the successful static gate; `OPT-B0-GAMMA ... --parent <own-SCF-directory>` uses only that new final density. This task consumed the sole SCF attempt with a parser failure; Gamma is BLOCKED_PARENT. Do not repeat these actions without new authorization. Public arithmetic: `python3.12 -B benchmarks/soc-core-memory-v1/replay_delivery.py --root .` (strict `--require-endpoints` exits 1). See [plan](../benchmarks/soc-core-memory-v1/README.md), [runtime](../docs/soc-core-runtime.md) and [actual status](../results/soc-core-memory/README.md). Ordinary invocations retain legacy behavior.

- Phase 8C: `si_qe_reference_evidence.py preflight --output .work/phase8c/<unique-manifest>.json` authenticates both K6/K8 inputs/builds and independent Python geometry/resources; no QE numerical worker. `run_si_qe.py Q-SCF <unique-directory> --qe-reference-profile K6 --preflight-manifest <manifest>` uses the explicit QE-only type (K8 likewise). Own-parent Γ uses `Q-GAMMA ... --parent <own-SCF-directory>`. Exactly four once-only slots, serial, on the clean execution commit; no DFTK or D/Q placeholder. `si_qe_reference_evidence.py export --index <four-slot-index>` publishes only its explicit whitelist; `... replay` reparses all public native gzip data and recomputes the trend. [Plan](../benchmarks/si-soc-k-reference-v1/README.md); [status](../results/si-soc-k-reference/README.md). The four formal slots are now consumed; review uses the public `replay` action. The archive-backed export index distinguishes `expected_execution_commit` from its four `profiles` entries; actual scientific execution remains0af11bac and replay tooling is separately bound. No additional numerical invocation is authorized.
